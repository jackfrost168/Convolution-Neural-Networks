...
pooling1 = tf.nn.max_pool(x1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
#这里操作后变成14x14x64

x2 = convolutional_block(X_input=pooling1, kernel_size=3, in_filter=64,  out_filters=[64, 64, 128], stage=2, block='b', stride=1)
pooling2 = tf.nn.max_pool(x2, ksize=[1, 2, 2, 1],strides=[1, 2, 2, 1], padding='SAME')
#上述conv_block操作后，尺寸变为7x7x128

x3 = identity_block(pooling2, 3, 128, [64, 64, 128], stage=2, block='c' )
pooling3 = tf.nn.max_pool(x3, [1, 2, 2, 1], strides=[1,2,2,1], padding='SAME')
#上述操作后张量尺寸变成4*4*128
